...
def setNumber(m, n, p, pos, v, logic="defined"):
    ...
    x, y = pos
    m[0][0] = m[0][0] + 1  # The total assigned number in the matrix
    m[x][y] = v
    m[x][0] = m[x][0] + 1
    m[0][y] = m[0][y] + 1

    n[v][0] = n[v][0] + 1  #n[1..9][0] record every number(1..9)'s assigned numbers
    #print( pos, v, n[v][0])
...
# Only Check the Vertical's amount is 9
def isDone(m):
    flag = True
    for i in range(1, 10):
        if m[0][i] != 9:
            ...
            for j in range(1, nums):
                ...
                for b in effectBox:
                    if not isInBox(m, p, b, number):
                        ...
                        """else:
                            grp = GetIndirectNumberInBox(possiblePos)
                            if grp!=(0, 0):
                                k = CheckInObvious(m, n, p, number, [(grp, possiblePos)])
                                # if set number in CheckInObvious, call self again
                                if k > 0:
                                    amt = amt + k
                                    return CheckObvious(m, n, p, amt=amt)"""
    return amt
        ...
        for grp in groups:
            ...
            for b in effectBox:
                if not isInBox(m, p, b, number):
                    ...
                    pos2 = copy.deepcopy(possiblePos)
                    for pos in pos2:
                        if not isPossibleByX(m, pos, number, groups=groups) or not isPossibleByY(m, pos, number,
                                                                                                 groups=groups):
                            possiblePos.remove(pos)
                    if len(possiblePos) == 1:
                        # if only one possible pos means it is the position of the number
                ...
                for b in effectBox:
                    if not isInBox(m, p, b, number):
                        ...
                        if len(possiblePos) == 1:
                            ...
                            setNumber(m, n, p, possiblePos[0], number,
                                      logic="InObviousWithGroup for {0}".format(number))
                ...
                for b in effectBox:
                    if not isInBox(m, p, b, number):
                        ...
                        if len(possiblePos) == 1:
                            ...
                            amt = amt + 1
                            setNumber(m, n, p, possiblePos[0], number,
                                      logic="InObviousByTwoGroup for {0}".format(number))
    return amt
    ...
    for pos, numbers in possibles:
        ...
        for i in range(1, 3):
            ...
            if len(r0) > 0 and len(r1) > 0:
                reduces, sets = CheckResultByTwoPossibleVal(m, n, p, r0, r1,
                                                            logic="CheckResultByTwoPossibleVal of {0} at the Pos {1}".format(
                                                                p[x][y], pos))
                if reduces > 0 or sets > 0:
                    return reduces, sets
    return 0, 0
    ...
    for num in range(1, 10):
        for i in range(1, 4):
            for j in range(1, 4):
                ...
                if len(r0) > 0 and len(r1) > 0:
                    reduces, sets = CheckResultByTwoPossibleVal(m, n, p, r0, r1,
                                                                logic="CheckResultByTwoPosition of {0}".format(
                                                                    possible))
                    if reduces > 0 or sets > 0:
                        return reduces, sets
    return 0, 0
...
def ReduceByTwoPositionOfLine(m, n, p, direction="x"):
    for num in range(1, 10):
        for i in range(1, 10):
            ...
            if len(r0) > 0 and len(r1) > 0:
                reduces, sets = CheckResultByTwoPossibleVal(m, n, p, r0, r1,
                                                            logic="CheckResultByTwoPosition of {0}".format(possible))
                if reduces > 0 or sets > 0:
                    return reduces, sets
    return 0, 0
